...
import { getAllExerciseBodyPart } from "../store/action/bodyPartExercise";
...
const EachBodyPage = () => {
  ...
  useEffect(() => {
    dispatch(getAllExerciseBodyPart(name));
  }, []);

  const state = useSelector((state) =>
    splitArray(state.bodyPartExercises[0], 40)
  );
  const all = useSelector((state) => state.bodyPartExercises);
  ...
  return (
    <div className=" col-12 pt-5 mt-5 row aboutPage">
      ...
      <div>
        <h4 className=" ms-5 aboutLin">Total - {all[0]?.length}</h4>
      </div>

      {isLoading ? (
        <h3 className=" p-5 m-5 text-center aboutLin">Loading</h3>
      ) : (
